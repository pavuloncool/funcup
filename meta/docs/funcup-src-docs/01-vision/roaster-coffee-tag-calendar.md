...
8. Na wszystkich stronach w przeglądarce użyj stylów spójnych z `product/apps/web/src/theme/authScreenStyles.ts` (tokeny jak consumer / auth).
9. Dla web: struktura tras w `product/apps/web/app/` (Next.js App Router); roaster tag: np. `/tag`.
...
4. `roaster` klika `btn-add-coffee` i przechodzi na stronę tagu (`/tag` w Next.js).
5. Na stronie `/tag` `roaster` uzupełnia `roaster-coffee-tag`, a wprowadzone dane zapisują się w tabeli Supabase.