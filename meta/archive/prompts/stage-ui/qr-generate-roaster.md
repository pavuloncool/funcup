...
Aby wygenerować kod QR, Roaster wchodzi na stronę `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx) i uzupełnia metrykę produktu `roaster-coffee-tag`.
...
Na stronie `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx), obok formularza `roaster-coffee-tag` pokazuje się img/png wygenerowanego kodu QR oraz przycisk `download-QR-svg`.
...
2. Wygeneruj stronę `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx), zawierającą wszystkie pola danych do wprowadzania i przechowywania informacji `roaster-coffee-tag`.
...
7. Podłącz stronę `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx) do nawigacji przyciskiem `btn-add-coffee`.
8. Na wszystkich stronach w przeglądarce użyj stylów spójnych z `product/apps/web/src/theme/authScreenStyles.ts` (tokeny jak `consumer-mobile` / auth).
...
4. `roaster` klika `btn-add-coffee` i przechodzi na stronę `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx).
5. Na stronie `roaster-add-coffee.tsx` (po zmianach: product/apps/web/app/tag/page.tsx) `roaster` uzupełnia `roaster-coffee-tag`, a wprowadzone dane zapisują się w tabeli Supabase.