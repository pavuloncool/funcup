...
todos:
  ...
  - id: tag-insert-auth
    ...
    status: completed
  - id: api-qr-route
    content: "product/apps/web/app/api/qr/route.ts (nodejs): auth, load tag, ownership, qrcode lib, JSON svg + png base64 + url"
    status: completed
  - id: tag-page-ui
    content: "product/apps/web/app/tag/page.tsx: layout per DoD (SVG prawo ≥480px / pod <480px), flow Zapisz → Generuj, preview SVG + opcjonalnie PNG/download; i18n-ready; nie łamać AppOpenGate/AnimatedSplash"
    status: completed
  - id: scan-qr-tag
...
Zgodnie z [qr-generate-roaster.md](meta/docs/Stage%20UI%20prompts/qr-generate-roaster.md): **QR jest jedynym nośnikiem informacji** między palarnią (web) a konsumentem (iOS/Android). Generator musi być **self-hosted w funcup web** ([qr gen prompt 2.md](meta/docs/Stage%20UI%20prompts/qr%20gen%20prompt%202.md)), a kod w QR musi prowadzić do adresu, który **konsument już obsługuje** przez deep link `funcup://q/{hash}` i ekran [product/apps/consumer-mobile/app/q/[hash].tsx](product/apps/consumer-mobile/app/q/[hash].tsx) → [product/apps/consumer-mobile/app/coffee/[id]/index.tsx](product/apps/consumer-mobile/app/coffee/[id]/index.tsx) + [product/packages/shared/src/hooks/useCoffeePage.ts](product/packages/shared/src/hooks/useCoffeePage.ts) (`scan_qr`).
...
- Zaktualizować typy: [product/packages/types/src/roasterCoffeeTag.ts](product/packages/types/src/roasterCoffeeTag.ts), [product/supabase/types/database.ts](product/supabase/types/database.ts) (regeneracja jeśli macie skrypt), [clientFormValuesToInsert](product/packages/shared/src/validation/roasterCoffeeTagForm.ts) — **nie** wkładać `public_hash` z klienta; serwer/DB generuje.

## 2. `POST /api/qr` (Next.js, Node runtime)

- Plik: [product/apps/web/app/api/qr/route.ts](product/apps/web/app/api/qr/route.ts) (folder utworzyć).
...
- Biblioteka QR: np. `qrcode` (PNG buffer + SVG string) — dodać zależność w [product/apps/web/package.json](product/apps/web/package.json).
- Błędy: 404 (brak tagu), 403 (cudzy tag), 400 (walidacja).

## 3. UI roaster: [product/apps/web/app/tag/page.tsx](product/apps/web/app/tag/page.tsx)
...
- **[product/supabase/functions/qr/scan-qr.ts](product/supabase/functions/qr/scan-qr.ts)** (lub entrypoint [product/supabase/functions/scan_qr](product/supabase/functions) jeśli to inny plik — zweryfikować faktyczny deploy): dodać ścieżkę „tag po `public_hash`”, zwracać JSON zrozumiały dla shared hooka.
- **[product/packages/shared/src/hooks/useCoffeePage.ts](product/packages/shared/src/hooks/useCoffeePage.ts)**: typ zwracany jako union; `queryFn` zwraca zunifikowany model lub dwa typy i `CoffeePage` rozgałęzia.
- **[product/apps/consumer-mobile/app/coffee/[id]/index.tsx](product/apps/consumer-mobile/app/coffee/[id]/index.tsx)**: render dla `kind === 'tag'` (etykieta, pochodzenie, obróbka, data wypału itd. z kolumn tagu); dla batch — istniejący layout.
- **[product/apps/web/app/q/[hash]/page.tsx](product/apps/web/app/q/[hash]/page.tsx)**: to samo rozróżnienie co mobile (lub uproszczony widok tagu).

## 5. Wycofanie `generate_qr` (Edge)

- Usunąć lub wyłączyć funkcję w [product/supabase/config.toml](product/supabase/config.toml) i katalog `product/supabase/functions/generate_qr`.
- Zastąpić lub usunąć [product/apps/web/app/dashboard/coffees/[id]/batches/[batchId]/QRDownloadButton.tsx](product/apps/web/app/dashboard/coffees/[id]/batches/[batchId]/QRDownloadButton.tsx) (pełne zastąpienie zaakceptowane na tym etapie).
- Zaktualizować [product/apps/web/tests/generate-qr.spec.ts](product/apps/web/tests/generate-qr.spec.ts) → testy przeciwko `POST /api/qr` (lokalny Next + mock auth / seed roaster + tag).
...
- Krótka aktualizacja kontraktu w `meta/docs/specs` (opcjonalnie), żeby nie rozjeżdżać się z [edge-function-generate-qr.md](meta/docs/specs/002-qr-coffee-platform/contracts/edge-function-generate-qr.md).
...
1. **Animated-splash (kontrakt obowiązkowy)**  
   - Aplikacja web startuje tak jak dziś: użytkownik widzi [AnimatedSplash](product/apps/web/components/AnimatedSplash.tsx) w ramach [AppOpenGate](product/apps/web/components/AppOpenGate.tsx) (pierwsze zamontowanie root layoutu → `onFinish` → dalsza nawigacja).  
