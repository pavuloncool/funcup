...
- Istnieja ekrany dashboardu roastera:
  - `product/apps/web/app/dashboard/coffees/page.tsx`
  - `product/apps/web/app/dashboard/coffees/[id]/page.tsx`
  - `product/apps/web/app/dashboard/coffees/[id]/batches/[batchId]/page.tsx`
- Istnieje flow danych dla batchy i tasting log:
  - `product/supabase/functions/coffee/log-tasting`
  - `product/packages/shared/src/services/tastingService.ts`

### Shared / testy

- Istnieje baseline testowy i integration coverage:
  - `product/packages/shared/src/hooks/us3IntegrationSmoke.test.ts`
  - `product/packages/shared/src/services/offlineTasting.integration.test.ts`

## 4) Scope implementacyjny Task-by-Task (Phase 008)

- `T075` `product/apps/web/dashboard/analytics/[batchId]/page.tsx`
...
- Regression:
  - rerun US5 tests (`product/packages/shared` + mobile typecheck),
...
- **Quality gate (shared + mobile):** `pnpm -C product/packages/shared test`, `pnpm -C product/packages/shared typecheck`, `pnpm -C product/apps/consumer-mobile typecheck` → PASS.
- **Hardening / regresja US2 (web):** `pnpm -C product/apps/web test:generate-qr`, `pnpm -C product/apps/web test:e2e` → PASS przy lokalnym Supabase (`supabase start`; klucze w `product/apps/web/.env.local` lub `./product/apps/web/product/scripts/sync-supabase-env-local.sh`).
- **US6:** integracja `product/packages/shared/src/analytics/roasterBatchAnalytics.test.ts` (mock tastings + filtr); ekran `product/apps/web/app/dashboard/analytics/[batchId]`.
