...
- Quality gate US6 (shared + mobile) jest zielony:
  - `pnpm -C product/packages/shared test` -> PASS
  - `pnpm -C product/packages/shared typecheck` -> PASS
  - `pnpm -C product/apps/consumer-mobile typecheck` -> PASS
- Regression gate US2 po zmianach US6 (lokalnie, Supabase + `.env.local`):
  - `pnpm -C product/apps/web test:generate-qr` -> PASS
  - `pnpm -C product/apps/web test:e2e` -> PASS
...
- Wspolne hooki i serwisy w `product/packages/shared`.

### Testy

- Baseline: `product/packages/shared` (Vitest), Playwright web (`product/apps/web/tests/`).
...
- Regresja: `product/packages/shared` test + typecheck, `product/apps/consumer-mobile` typecheck.
- Regresja US2: `pnpm -C product/apps/web test:generate-qr`, `pnpm -C product/apps/web test:e2e` (Supabase lokalnie).
...
./product/apps/web/product/scripts/sync-supabase-env-local.sh   # opcjonalnie: klucze do product/apps/web/.env.local
pnpm -C product/apps/consumer-mobile start
...
- CI/local: `pnpm -C product/packages/shared test`, `pnpm -C product/packages/shared typecheck`, `pnpm -C product/apps/consumer-mobile typecheck`, `pnpm -C product/apps/web test:generate-qr`, `pnpm -C product/apps/web test:e2e`.
...
| Gate | Komenda | Wynik |
|------|---------|--------|
| Shared tests | `pnpm -C product/packages/shared test` | PASS (19 testów) |
| Shared types | `pnpm -C product/packages/shared typecheck` | PASS |
| Mobile types | `pnpm -C product/apps/consumer-mobile typecheck` | PASS |
| US2 generate_qr | `pnpm -C product/apps/web test:generate-qr` | PASS |
| US2 E2E | `pnpm -C product/apps/web test:e2e` | PASS (przy `WEB_BASE_URL=http://127.0.0.1:3000` i działającym `next dev -p 3000`; Playwright ładuje `.env.local` — ustaw spójny port lub nadpisz `WEB_BASE_URL`) |
| Web production build (perf/hardening baseline) | `pnpm -C product/apps/web build` | PASS |
