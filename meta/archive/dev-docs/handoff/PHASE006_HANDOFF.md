...
- Istnieja ekrany i komponenty, ktore beda rozszerzane:
  - `product/apps/consumer-mobile/app/coffee/[id]/components/FlavorNoteSelector.tsx`
  - `product/apps/consumer-mobile/app/coffee/[id]/CoffeePageCommunity.tsx`
- Reputation thresholds sa dostepne w shared:
  - `product/packages/shared/src/constants/reputationThresholds.ts`

### Shared / data

- Istnieja hooki i services pod log tasting oraz discovery:
  - `product/packages/shared/src/services/tastingService.ts`
  - `product/packages/shared/src/hooks/*`
- Fundament pod testy integracyjne jest gotowy (vitest w `product/packages/shared`).
...
- Regression:
  - rerun US3 tests (`product/packages/shared` + mobile typecheck),
