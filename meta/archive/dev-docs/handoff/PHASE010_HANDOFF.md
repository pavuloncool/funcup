...
| **010-002** | DONE | **Vite:** [product/apps/frontend/src/AnimatedSplash.jsx](product/apps/frontend/src/AnimatedSplash.jsx) + `public/assets/home-print.svg`, `home-bean.svg`; [product/apps/frontend/src/App.tsx](product/apps/frontend/src/App.tsx) (`ready` + `<Routes>`). **Next:** [product/apps/web/components/AnimatedSplash.tsx](product/apps/web/components/AnimatedSplash.tsx), [product/apps/web/components/AppOpenGate.tsx](product/apps/web/components/AppOpenGate.tsx), [product/apps/web/app/layout.tsx](product/apps/web/app/layout.tsx), assety w `product/apps/web/public/assets/`. **Frontend:** nawigacja z home — **`<Link to>`** (react-router), **nie** `<a href>`, żeby uniknąć pełnego reloadu i ponownego splashu ([product/apps/frontend/src/pages/HomePage.tsx](product/apps/frontend/src/pages/HomePage.tsx)). **Next:** strona główna jak Vite + trasy `/scan`, `/hub`, `/profile`. **Expo (010-002):** pierwotnie [product/apps/consumer-mobile/app/index.tsx](product/apps/consumer-mobile/app/index.tsx) → redirect `/home`; **010-003** zastąpiło entry przez [MobileEntrySplash](product/apps/consumer-mobile/src/components/entry/MobileEntrySplash.tsx). |
...
1. Dodać w `product/apps/consumer-mobile` **jeden** komponent / ekran entry (np. `app/index.tsx` zamiast samego `Redirect`, lub grupa tras), który:
...
pnpm -C product/apps/consumer-mobile lint
```

Opcjonalnie regresja web (nie jest strict częścią 010-003, ale zdrowy nawyk):

```bash
pnpm -C product/apps/web lint
pnpm -C product/apps/frontend lint
```
...
```bash
pnpm install
pnpm -C product/apps/consumer-mobile start          # Expo
pnpm -C product/apps/frontend dev          # Vite :5173
pnpm -C product/apps/web dev               # Next :3000
```
