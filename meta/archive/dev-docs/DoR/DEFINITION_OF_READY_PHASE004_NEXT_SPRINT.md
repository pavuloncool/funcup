...
- [x] Lokalnie działa co najmniej:
  - [x] `pnpm -C product/apps/web dev`
  - [x] `pnpm -C product/apps/consumer-mobile start`
...
- `pnpm install`: PASS
- `pnpm -C product/apps/web dev`: PASS
- `pnpm -C product/apps/consumer-mobile start`: PASS (Metro starts on `http://localhost:8081`)
...
- [x] Brak krytycznych błędów TypeScript w nowych modułach (product/apps/web, product/apps/consumer-mobile, product/packages/shared).
...
- `pnpm lint`: PASS
- `pnpm test`: PASS
- `pnpm -C product/apps/consumer-mobile typecheck`: PASS (po poprawce `supabaseClient.ts`)
- `pnpm -C product/packages/shared typecheck`: PASS
- `pnpm exec tsc -p product/apps/web/tsconfig.json --noEmit`: PASS (po poprawce `app/layout.tsx`)
...
- [x] Konwencja ścieżek dla web (Phase 4) została doprecyzowana: **canonical path = `product/apps/web/app/...`** (Next.js App Router).
...
- [x] `product/supabase/seed.sql` uruchamia się bez błędów krytycznych w środowisku deweloperskim.
  - Status 2026-04-08: PASS po uruchomieniu `supabase start --exclude logflare,vector,studio,imgproxy,mailpit`.
- [x] Typy DB (`product/supabase/types/database.ts`) są zsynchronizowane z aktualnym schematem.
- [ ] Edge functions wymagane dla US1 istnieją i są testowalne:
  - [x] `scan_qr` istnieje (`product/supabase/functions/scan_qr/index.ts`)
  - [x] `update_coffee_stats` istnieje (`product/supabase/functions/update_coffee_stats/index.ts`)
...
- [ ] Edge function dla US2 (`generate_qr`) istnieje i jest testowalna:
  - [x] `product/supabase/functions/generate_qr/index.ts` — zaimplementowana (T043)
  - [x] Smoke runtime: PASS (`201` create + `404` not_found + `403` forbidden + idempotent `200`)

Evidence T017/T018:
- `product/supabase/EVIDENCE_T017_T018.md`
...
- [x] Shared (`product/packages/shared`) udostępnia:
...
| T044 web auth login | Web Auth Owner | 1 dzień | T021, baza `product/apps/web` (T004) | Strona logowania działa; email auth flow + walidacja formularza; poprawne stany loading/error. |
...
- Peer dependency drift w `product/apps/consumer-mobile` (Expo compatibility warnings) — monitorować, nie blokuje Phase 4 web.
...
- [ ] smoke `product/supabase/seed.sql` oraz runtime edge functions jest wykonany na środowisku dev.
- [x] smoke `product/supabase/seed.sql` oraz runtime edge functions jest wykonany na środowisku dev.
...
- Dodano skrypt: `product/scripts/phase004-readiness-check.sh`
- Uruchamianie: `pnpm phase004:readiness`
