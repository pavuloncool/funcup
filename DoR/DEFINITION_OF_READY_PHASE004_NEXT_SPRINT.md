...
- `pnpm -C apps/mobile start`: PASS (Metro starts on `http://localhost:8081`)
...
- [x] Brak krytycznych błędów TypeScript w nowych modułach (apps/web, apps/mobile, packages/shared).
...
- `pnpm lint`: PASS
- `pnpm test`: PASS
- `pnpm -C apps/mobile typecheck`: PASS (po poprawce `supabaseClient.ts`)
...
- Peer dependency drift w `apps/mobile` (Expo compatibility warnings) — monitorować, nie blokuje Phase 4 web.
...
- Dodano skrypt: `scripts/phase004-readiness-check.sh`
