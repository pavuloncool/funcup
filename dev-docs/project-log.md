...
- Mobile: `RootErrorBoundary`, stany loading (skeleton), empty/error (Journal, Hub discovery, Learn), Coffee Page oparta o `useCoffeePage` / `scan_qr`, deep link `apps/consumer-mobile/app/q/[hash].tsx` (par z web `/q/{hash}`), `eas.json` (profil production), Android intent filter dla `funcup` / host `q`.
